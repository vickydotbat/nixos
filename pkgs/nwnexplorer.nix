...
let
  nwnInstallDir =
    "${config.home.homeDirectory}/.local/share/Steam/steamapps/common/Neverwinter Nights";

  toWineZPath = path:
    "Z:${lib.replaceStrings [ "/" ] [ "\\\\" ] path}";

  nwnDirectoryWine = "${toWineZPath nwnInstallDir}\\\\";

  nwnexplorerReg = pkgs.writeText "nwnexplorer.reg" ''
  ...
  nwnexplorer = pkgs.stdenvNoCC.mkDerivation rec {
    pname = "nwnexplorer";
    version = "1.8.5";
    assetVersion = "185";
...
${pkgs.wineWow64Packages.staging}/bin/wine regedit /S ${nwnexplorerReg}
